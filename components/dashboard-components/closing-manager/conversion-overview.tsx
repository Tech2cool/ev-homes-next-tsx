...
function Ring({
  ...
  return (
    <>
      {/* background track */}
      <circle cx="112" cy="112" r={r} stroke="#e5e7eb" strokeWidth={strokeWidth} fill="transparent" />
      {/* value arc */}
      <circle
        cx="112"
        cy="112"
...
export function ConversionOverview({ leads, bookings, visits }: ConversionOverviewProps) {
  ...
  const overview = [
    ...
    { title: "Bookings", percentage: Math.round(bookingRate * 100), color: "#8b5cf6" },
    { title: "Visits", percentage: Math.round(visitRate * 100), color: "#06b6d4" },
  ]

  return (
    <div className="">
      {/* Title for right */}
      <h2 className="text-xl font-semibold text-foreground mb-6">Conversion Overview</h2>
            ...
            <div className="relative w-78 h-78 ml-6 ">
              ...
              <div className="absolute inset-0 mb-10 mr-7  flex items-center justify-center">
                <div className="text-center">
                  <div className="text-2xl font-bold text-gray-900">{safeLeads.toLocaleString()}</div>
                  <div className="text-xs font-medium text-gray-600 mt-0">TOTAL LEADS</div>
                </div>
              </div>
            </div>
          </div>
            ...
            {overview.map((kpi, index) => (
              <div key={index} className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-gray-600 uppercase tracking-wide">{kpi.title}</span>
                </div>
                <span className="text-base ml-5 font-bold text-gray-900">{kpi.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
